...
public class HeapSort {
    ...
    public int[] myHeapSort1(int[] arr) {
       /* 自底向上建堆
        * 时间复杂度O(n)
        * s=∑2^(i-1)*(h-i)堆高度为h，第i层有2^(i-1)个节点，每个节点判断(h-i)次,i从h-1层到1层
        * S=2^(h-2)*1+2^(h-3)*2+2^(h-4)*3+...+2^(1)*(h-2)+2^(0)*(h-1)
        * 2S=2^(h-1)*1+2^(h-2)*2+2^(h-3)*3...+2^(2)*(h-2)+2^(1)*(h-1)
        * 2S-S=S=2^(h-1)*1+2^(h-2)+...+2^2+2^1-(h-1)=2^h-h-1
        * 等比数列求和：Sn=a1*(1-q^n)/(1-q)=2*(1-2^(h-1))/(1-2)=2^h-2
        * 有2^h个节点，设为N
        * S=N-logn-1
        * 时间复杂度为O(N)
        */
...
*/
     ...
     * 时间复杂度O(nlogn)
     * 证明：
     * 第k层有2^(k-1)个节点，遍历k次，共h层
     * S = 1+2*(2^1)+3*(2^2)+4*(2^3)+5*(2^4)+...+h*(2^(h-1))
     * 2S= 0+1*(2^1)+2*(2^2)+3*(2^3)+4*(2^4)+...+(h-1)*(2^(h-1))+h*(2^(h))
     * 2S-S=-1-2-2^2-2^3-...-2^(h-1)+h*(2^(h))
     * -1-2-2^2-2^3-...-2^(h-1)=-（1*（1-2^（h））/(1-2)=-(2^h-1)≈-2^h
     * s=(h-1)*（2^h），堆共有2^h个节点，设为N
     * S和N的关系：NlogN
     */
